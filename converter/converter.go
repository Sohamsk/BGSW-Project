...
func handleBody(rules []json.RawMessage, fromSub bool) string {
	...
	for _, rule := range rules {
		...
		if raw.RuleType == "FuncStatement" || raw.RuleType=="SubStatement" || raw.RuleType=="EnumerationRule" || raw.RuleType=="PropertyGetStatement" || raw.RuleType=="PropertySetStatement" || raw.RuleType=="PropertyLetStatement"|| raw.RuleType=="TypeStmtRule" {
			continue
		}
...
func handleoutsidestmts(rules []json.RawMessage) string {
	...
	for _, rule := range rules {
		...
		if raw.RuleType == "FuncStatement" || raw.RuleType=="SubStatement" || raw.RuleType == "DeclareVariable" || raw.RuleType=="EnumerationRule" || raw.RuleType=="PropertyGetStatement" || raw.RuleType=="PropertySetStatement" || raw.RuleType=="PropertyLetStatement" || raw.RuleType=="TypeStmtRule"{
			inter, err := ConvertRule(rule)
			if err == nil {
